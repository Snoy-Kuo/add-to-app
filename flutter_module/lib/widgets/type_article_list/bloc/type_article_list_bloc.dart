...
class TypeArticleListBloc
    extends Bloc<TypeArticleListEvent, TypeArticleListState> {
  final TypeArticleRepo repository;
  List<NewsItem>? list = [];
  NewsType? type;

  TypeArticleListBloc({required this.repository})
      : super(TypeArticleListLoading());
  ...
  Stream<TypeArticleListState> _mapRefreshTypeArticleListToState(
      NewsType type) async* {
    try {
      yield TypeArticleListLoading();
      list = await repository.fetchArticleList(type: type);
      this.type = type;
      yield TypeArticleListLoaded(
          list: (list == null) ? [] : list!.sublist(0, 2), type: type);
    } catch (e) {
      LogUtil.e('e=$e');
